...
<<<<<<< HEAD
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:root@localhost/violation'
=======
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:root@localhost/violation'
>>>>>>> c0223a74534e4e79be2f6341ed2358f5e70cf16a
...
<<<<<<< HEAD
model = load_model('model_8class.h5', compile=False)
...
<<<<<<< HEAD
    ...
    for frame_index in frame_indices:
        ...
        video_stream.set(cv2.CAP_PROP_POS_FRAMES, int(frame_index))
    ...
    # Require the same violent activity
    # to appear in at least 20% of sampled frames.
    detection_count = max(
        2,
        int(len(predictions) * 0.20)
    )

    detected_activities = [
        activity
        for activity, count in activity_counts.items()
        if activity in VIOLENT_CLASSES
        and count >= detection_count
    ]

    print("--------------------------------")
    print("Sampled frames:", len(predictions))
    print("Activity counts:", activity_counts)
    print("Detected violent activities:", detected_activities)
    print("--------------------------------")
